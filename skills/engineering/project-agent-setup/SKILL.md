...
description: "Scaffold per-repo agent config (GitHub or local-markdown issue tracker, triage labels, domain doc layout) that the engineering skills (`to-tickets`, `to-spec`, `wayfinder`, `triage`, `diagnosing-bugs`, `tdd`, `improve-codebase-architecture`) consume. Writes an `## Agent skills` block to `AGENTS.md` and seeds `docs/agents/`. Run once per repo before first use of the other engineering skills, or if they appear to be missing config about the issue tracker, triage labels, or domain docs."
...
Never create `AGENTS.md` if it would conflict with an existing file; always edit the one that's already there.
