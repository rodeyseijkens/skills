...
- **`/create-pr`** — PR title and categorized description from the branch diff. Opens the PR when asked.
- **`/review-comment-triage`** — work through unresolved review comments on one or more PRs: validate each finding against the stack tip (or the PR's branch), post concise replies, resolve addressed threads, and fix approved findings with atomic commits. Reach for it when a PR — or a stack of them — has accumulated review comments to clear.
