...
- **`/grill-me`** is the same relentless interview as `/grill-with-docs`, but **stateless**: it saves nothing locally and builds no `CONTEXT.md`. Reach for it when you are **not working in a working directory**, sharpening a plan, a design, a piece of writing, anything with no repo under it. If you are in a working directory, use `/grill-with-docs` instead: it runs the same interview and leaves a paper trail, so it is strictly the better one.
...
- **`/create-pr`** writes the PR title and categorized description from the branch diff. Opens the PR when asked.
- **`/review-comment-triage`** works through unresolved review comments on one or more PRs: validate each finding against the stack tip (or the PR's branch), post concise replies, resolve addressed threads, and fix approved findings with atomic commits. Reach for it when a PR, or a stack of them, has accumulated review comments to clear.
- **`/resolving-merge-conflicts`** works an in-progress merge or rebase conflict hunk by hunk, resolving by **intent** traced to each side's primary source rather than by picking lines, then finish the operation. It never runs `--abort`. Standalone and off every flow: reach for it when you are already mid-conflict.
...
- **`/teach`** learns a concept over multiple sessions, using the current directory as a stateful workspace.
- **`/todo`** queues a new item into the current session's todo list while a task is already underway. The agent weighs it against the running task and either **preempts** (starts it now, then resumes the suspended task) or **places** it at the slot its urgency earns. Reach for it whenever something new lands mid-task and you want it tracked at the right priority.
